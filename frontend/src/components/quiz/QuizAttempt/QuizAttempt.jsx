...
export const QuizAttempt = () => {
      ...
      try {
        ...
        if (attemptId) {
          ...
          if (existingQuiz) {
            ...
            // For reattempt, reset all answers
            ...
            quizData = {
              ...
              title: `${existingQuiz.quizType.toUpperCase()} Quiz - Reattempt`,
              isAttempted: false
            };
          }
        }
  ...
  const handleTextAnswerChange = (text) => {
    if (currentQ.questionType === 'short-answer' || currentQ.questionType === 'long-answer') {
      setUserAnswers(prev => ({ 
        ...prev, 
        [currentQuestion]: { 
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      await axios.post(
        `http://localhost:5000/api/quizzes/save/${documentId}/${attemptId}`,
        quizData,
        { 
          headers: { 
            Authorization: `Bearer ${token}`,
  ...
  const getQuestionTypeStats = () => {
    ...
    quiz.questions.forEach(question => {
      if (stats.hasOwnProperty(question.questionType)) {
        stats[question.questionType]++;
      }
    });
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-white border border-orange-200 rounded-xl p-4 text-center">
                  <div className="text-xl font-bold text-orange-600 capitalize">{quiz.quizType || 'Mixed'}</div>
              ...
              <div className="bg-blue-50 rounded-xl p-4 mb-6 border border-blue-200">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {Object.entries(questionStats).map(([type, count]) => (
                    count > 0 && (
                      <div key={type} className="bg-white rounded-lg p-3 text-center border border-blue-100">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 mb-4">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getQuestionTypeColor(currentQ.questionType)}`}>
                      {getQuestionTypeIcon(currentQ.questionType)} {currentQ.questionType.replace('-', ' ')}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Answer Input */}
              {currentQ.questionType === 'multiple-choice' && currentQ.options && currentQ.options.length > 0 ? (
              ...
              ) : (currentQ.questionType === 'short-answer' || currentQ.questionType === 'long-answer') ? (
                <div className="space-y-3">
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                    <label className="block font-medium text-gray-700 mb-2">
                      {currentQ.questionType === 'short-answer' ? '📝 Your Answer' : '📄 Detailed Answer'}
                    </label>
                    <textarea
                      value={currentUserAnswer?.type === currentQ.questionType ? currentUserAnswer.value : ''}
                      onChange={(e) => handleTextAnswerChange(e.target.value)}
                      placeholder={currentQ.questionType === 'short-answer' 
                      ...
                      className={`w-full p-3 border border-blue-300 rounded-lg focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition-all resize-none font-medium ${
                        currentQ.questionType === 'short-answer' ? 'h-20' : 'h-32'
                      }`}
                    />
